...
- **Edit only the file(s) named in the finding.** Do not touch anything else.
...
- **No git, no running the app, no analyzers or test suites.** Verification happens centrally after all fixers finish. Add or change tests only if the finding itself is about a test.
